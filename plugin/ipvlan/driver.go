...
var (
	gitRepoURL = "http://nerdalert:spam99@172.16.86.1/nerdalert/git-overlay.git"
)
...
func New(version string, ctx *cli.Context) (Driver, error) {
	...
	log.Debugf("The IPVlan master index is [ %s ]", IpVlanEthIface)
	...
	case ipVlanL3:
		...
		ipVlanMode = ipVlanL3
		containerGW = nil
		// Bind Git boostrap parameters
		if ctx.Bool("git-multihost") == true && ctx.String("repo") != "" {
			gitBootstrapURL = ctx.String("repo")
			gitPollInterval = ctx.Int("poll-interval")
			gitBoostrapBool = ctx.Bool("git-multihost")
		} else if ctx.Bool("git-multihost") == true && ctx.String("repo") == "" {
			log.Fatalf("A Git repository URL is required in order to bootsrap IPvlan via Git. View options with the help flag --help")
		}
		log.Debugf("Specified git repo is [ %s ]", gitBootstrapURL)
	...
	go control.Run(gitBootstrapURL, IpVlanEthIface, gitPollInterval, gitBoostrapBool)
...
func (driver *driver) createNetwork(w http.ResponseWriter, r *http.Request) {
	...
	if ipVlanMode == ipVlanL3 {
		...
		log.Debugf("The Network UUID is [ %s ] ", driver.network)
		log.Debugf("The IPVlan port is:\n [ %+v ] ", driver)
		// Call AnnounceEndpoint to generate a json file describing this endpoint
		control.AnnounceEndpoint(containerCidr, IpVlanEthIface, gitRepoURL)
	}
}
